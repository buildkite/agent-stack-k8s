...
type AgentConfig struct {
	...
	BootstrapTimeout *int `json:"bootstrap-timeout,omitempty"` // BUILDKITE_KUBERNETES_BOOTSTRAP_CONNECTION_TIMEOUT
}
...
func (a *AgentConfig) ApplyToCheckout(ctr *corev1.Container) {
	...
	if a.BootstrapTimeout != nil {
		appendToEnv(ctr, "BUILDKITE_KUBERNETES_BOOTSTRAP_CONNECTION_TIMEOUT", fmt.Sprintf("%d", *a.BootstrapTimeout))
	}
	a.applyHooksVolumeTo(ctr)
	a.applyPluginsVolumeTo(ctr)
}
