...
func (w *worker) Build(podSpec *corev1.PodSpec, skipCheckout bool, inputs buildInputs) (*batchv1.Job, error) {
	...
	// init containers do not run in parallel. Ideas for workarounds:
	//  * do the image pull preflight checks as regular containers in
	//    another pod, then use affinity to run the real workload on the same
	//    node
	// 	* figure out how to tell the current kubelet to pull images
	//    (use CRI ImageService.PullImage?)
	...
	// TODO: think of a way to re-use the podSpec's existing init containers to
	// detect their own image pull failure.
	...
	for image := range preflightImagePulls {
		...
		initContainers = append(initContainers, corev1.Container{
			...
			// Because Alpine Busybox is dynamically linked against musl!
			Command: []string{"/workspace/buildkite-agent"},
			Args:    []string{"--version"},
			VolumeMounts: []corev1.VolumeMount{{
				Name:      "workspace",
