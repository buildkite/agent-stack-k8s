...
func NewImagePullBackOffWatcher(logger *zap.Logger, k8s kubernetes.Interface) *imagePullBackOffWatcher {
	watcher := &imagePullBackOffWatcher{
		logger: logger,
		k8s:    k8s,
	}
	return watcher
...
func (w *imagePullBackOffWatcher) OnUpdate(old, new any) {
	...
	newPod, _ := new.(*v1.Pod)
	w.cancelImagePullBackOff(newPod)
}

func (w *imagePullBackOffWatcher) cancelImagePullBackOff(pod *v1.Pod) {
	...
	for _, containerStatus := range pod.Status.ContainerStatuses {
		if containerStatus.State.Waiting != nil && containerStatus.State.Waiting.Reason == "ImagePullBackOff" {
			log.Info("Pod is in ImagePullBackOff")
		}
	}
}
