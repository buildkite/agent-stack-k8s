...
func NewAgentClient(token, endpoint, clusterID, queue string, agentQueryRules []string, reseverationSupport bool) (*AgentClient, error) {
	if endpoint == "" {
	...
	return &AgentClient{
		...
		clusterID:          clusterID,
		queue:              queue,
		reservationSupport: reseverationSupport,
	}, nil
}
